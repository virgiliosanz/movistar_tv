...
#define debug_programme(prog) debug("Programme -> tit: '%s', chan: '%s' dir: '%s' country: '%s' start: '%02d/%02d/%04d %02d:%02d:%02d'\n'%s'", \
	bdata((prog)->title), bdata((prog)->channel), \
	bdata((prog)->director), bdata((prog)->country), \
...
#define debug_channel(chan) debug("Channel -> id: '%s', name: '%s', short: '%s' icon: '%s' url: '%s' ip: '%s' port: '%d'", \
	bdata((chan)->id), bdata((chan)->display_name), \
	bdata((chan)->short_name), bdata((chan)->icon), \
	bdata((chan)->url), \bdata((chan)->ip), (chan)->port);
