 ...
 */
static void
_conf_parse_platform_json(conf_s *cnf, const bstring json)
{
...
static void
_conf_parse_client_json(conf_s *cnf, const bstring json)
{
	const char *path_demarcation[] = { "resultData", "demarcation", (const char *)0 };
	const char *path_svPackages[] = { "resultData", "tvPackages", (const char *)0 };
	...
	v = yajl_tree_get(node, path_svPackages, yajl_t_string);
	check(v != NULL, "Error reading tvPackages (client json)");
	s = bfromcstr(YAJL_GET_STRING(v));
	debug("node: %s/%s = %s\n", path_svPackages[0], path_svPackages[1], s->data);
