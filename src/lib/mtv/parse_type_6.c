...
struct type_6_parser_context_s {
	bool     in_demarcation;
	char     *domain;
	struct mcast *mcast;

};
typedef struct type_6_parser_context_s type_6_parser_context_s;

#define _TYPE_1_PARSER_DOMAIN_FMT "DEM_%d.imagenio.es"

static xmlEntityPtr
_get_entity(type_6_parser_context_s *ctx, const xmlChar *name)
{
...
static void
_start_element(type_6_parser_context_s *ctx,
	       const xmlChar           *xn,
	       const xmlChar          **attrs)
{
...
static void
_error(type_6_parser_context_s *ctx, const char *msg, ...)
{
	warn_if(!ctx, "ctx is NULL");
...
static void
_warning(type_6_parser_context_s *ctx, const char *msg, ...)
{
	warn_if(!ctx, "ctx is NULL");
...
*/
struct mcast *
mtv_parse_file_type_6(const char *xml, const int demarcation)
{
	type_6_parser_context_s *ctx = NULL;

	error_if(NULL == xml, error, "Param Error");

	ctx = malloc(sizeof(type_6_parser_context_s));
