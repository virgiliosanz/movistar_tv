...
char *test_learn_yajl()
{
    ...
    node = yajl_tree_parse(client_json, err_buf, sizeof(err_buf));
    debug("Error parsing: %s", err_buf);
    mu_assert(NULL != node, "Error Parsing json")

    const char * path[] = { "resultData", "publicIP", (const char *) 0 };
    yajl_val v = yajl_tree_get(node, path, yajl_t_string);
    if (v) debug("%s/%s: %s\n", path[0], path[1], YAJL_GET_STRING(v));
    else   debug("no such node: %s/%s\n", path[0], path[1]);
